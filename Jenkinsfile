  ...
  stages {
    ...
    stage('Deploy') {
        ...
        steps {
            ...
            sh '""ssh -o StrictHostKeyChecking=no -tt -i $KEYFILE ubuntu@18.196.109.172 "rm -rf react1 && mkdir react1 && cd react1 && aws s3 sync s3://jenkins-pipeline-artifacts-gdm/react1-artifacts . && tar -xf react1.tar.gz && rm -rf react1.tar.gz" ""'
        }
      }
    }
  }
